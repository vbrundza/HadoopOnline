...
public class TestReduceTask extends TestCase {
  ...
  public void runValueIterator(Path tmpDir, Pair[] vals, 
    ...
    IFile.Writer<Text, Text> writer = 
      new IFile.Writer<Text, Text>(conf, rfs, path, Text.class, Text.class, codec, null);
    for(Pair p: vals) {
    ...
    RawKeyValueIterator rawItr = 
      Merger.merge(conf, rfs, Text.class, Text.class, codec, new Path[]{path}, false, conf.getInt("io.sort.factor", 100), tmpDir, new Text.Comparator(), new NullProgress(), null, null);
    @SuppressWarnings("unchecked") // WritableComparators are not generic
    ValuesIterator valItr = 
      new ValuesIterator<Text,Text>(rawItr,
